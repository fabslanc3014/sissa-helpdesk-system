...
import { Briefcase, Calendar, FileText, CheckCircle, AlertCircle, Clock, PauseCircle, RefreshCw } from "lucide-react";

interface CompanyTicketsReportViewProps {
  tickets: TicketRef[];
}

export default function CompanyTicketsReportView({ tickets }: CompanyTicketsReportViewProps) {
  ...
  const handleExport = () => {
    ...
    // Validate that a company selection has been made
    if (!selectedCompany) {
      setValidationError("Please select a company to export the report.");
      return;
    }
    ...
    const filtered = tickets.filter((t) => {
      // 1. Filter by Company
      const matchesCompany = selectedCompany === "ALL" || t.company === selectedCompany;
  ...
  return (
    <div className="flex-1 overflow-y-auto p-8 bg-[#F9FAFB] font-sans">
      ...
      <div className="border-b border-gray-200 pb-5 mb-6">
        <h2 className="text-4xl font-bold text-gray-805 tracking-tight animate-fade-in font-sans">Company Tickets</h2>
        <p className="text-xs text-gray-500 mt-1">
          Generate structured company ticket reports with configurable status routing queries.
        </p>
      </div>
        ...
        <div className="space-y-5 max-w-xl">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center">
            ...
            <div className="sm:w-2/3">
              <select
                id="report-tickets-company-select"
        ...
        <div className="border-t border-gray-100 pt-6 mt-6">
          {isGenerated && reportTickets ? (
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-gray-650 uppercase tracking-widest flex items-center">
                  ...
                  Tickets Registry for {selectedCompany === "ALL" ? "All Active Companies" : selectedCompany}
              ...
              {reportTickets.length > 0 ? (
                ...
                <div className="flex flex-col items-center justify-center p-12 text-gray-455 select-none bg-gray-50/50 rounded-lg border border-dashed border-gray-200">
                  <AlertCircle className="w-8 h-8 text-gray-350 stroke-1.25 mb-1.5" />
                  <p className="text-xs font-semibold">No matching support inquiries found</p>
                  <p className="text-[10px] text-gray-400">
                    No tickets found matching the specified parameters under {selectedCompany === "ALL" ? "All Companies" : selectedCompany}.
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="flex flex-col items-center justify-center p-12 text-gray-400 select-none min-h-[160px]">
              ...
              <p className="text-xs font-semibold">Company ticket logs status ready for compilation</p>
