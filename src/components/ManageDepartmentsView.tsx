...
import { Department } from "../manageData";
...
export default function ManageDepartmentsView({
  departments,
  onAddDepartment,
  onDeleteDepartment
}: ManageDepartmentsViewProps) {
  ...
  return (
    <div className="flex-1 overflow-y-auto p-6 bg-[#F9FAFB] font-sans">
      ...
      <div className="border-b border-gray-200 pb-4 mb-6">
        <h2 className="text-3xl font-extralight text-gray-800 tracking-tight">Departments</h2>
        <div className="text-xs text-gray-400 mt-1">Configure user departments to partition support routing metrics and operational divisions.</div>
      </div>
          ...
          <div className="p-4 bg-white border-b border-gray-150 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 text-xs text-gray-500">
            <div className="flex items-center space-x-2">
              ...
              <select
                ...
                className="bg-white border border-gray-350 rounded px-2 py-0.5 focus:outline-none"
              >
                ...
                {paginated.length > 0 ? (
                  paginated.map((d) => (
                    <tr key={d.id} className="hover:bg-gray-50/50 transition">
                      <td className="p-3 pl-5 font-semibold text-[#E15B39]">{d.name}</td>
                      <td className="p-3 font-semibold text-[#E15B39]">{d.description || "-"}</td>
