...
interface PersonnelStatusReportViewProps {
  tickets: TicketRef[];
}

export default function PersonnelStatusReportView({ tickets }: PersonnelStatusReportViewProps) {
  ...
  const handleGenerate = () => {
    ...
    if (!selectedPersonnel) {
      setValidationError("Please select a personnel member to generate the report.");
      return;
    }

    const targetPersonnel = selectedPersonnel === "ALL" ? REQUESTERS : [selectedPersonnel];

    const rows = targetPersonnel.map((personName) => {
      // Find all tickets for this requester
      ...
      // Map matching companies for real statistics
      let company = "External Partner";
      if (personName === "Lance Fabregas") company = "I686 Online Shop";
      else if (personName === "Isabella Torres") company = "Makati Logistics Hub";
      else if (personName === "Christian Santos") company = "Avasia Online Services";
      else if (personName === "Theresa Lim") company = "SISSA Corp Philippines";
      else if (personName === "Ramon Valenzuela") company = "I686 Online Shop";
    ...
    <div className="flex-1 overflow-y-auto p-8 bg-[#F9FAFB] font-sans">
      ...
      <div className="border-b border-gray-200 pb-5 mb-6">
        ...
        <p className="text-xs text-gray-500 mt-1">
          Detailed metrics ledger and caseload distribution for company personnel and client employees.
        </p>
      </div>
      ...
      <div className="bg-white border border-gray-200 shadow-sm rounded-lg p-8 space-y-6">
        ...
        <div className="space-y-4 max-w-xl">
          {/* 1. Personnel Selection Dropdown */}
          ...
          <div className="flex flex-col sm:flex-row sm:items-center">
            ...
            <div className="sm:w-2/3">
              <input
                id="report-personnel-start-date"
                type="date"
                value={startDate}
        ...
        <div className="pt-2">
          <button
            id="btn-personnel-report-generate"
            type="button"
        ...
        <div className="border-t border-gray-100 pt-6 mt-6">
          {isGenerated && reportData ? (
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-gray-650 uppercase tracking-widest flex items-center">
                  ...
                  Personnel Metrics for {selectedPersonnel === "ALL" ? "All Active Personnel" : selectedPersonnel}
                ...
                <span className="text-[10px] font-bold text-gray-500 bg-gray-50 px-2.5 py-1 rounded border border-gray-150">
                  Personnel count: {reportData.length}
                </span>
              </div>
                  ...
                  <table className="w-full text-left border-collapse text-xs">
                    <thead>
                      <tr className="bg-gray-55 border-b border-gray-200 font-semibold text-gray-500 uppercase text-[10px] tracking-wider select-none">
                        ...
                        <th className="p-3">Company Represented</th>
                      ...
                      {reportData.map((row) => (
                        ...
                        >
                          <td className="p-3 pl-6 font-semibold text-gray-900 flex items-center gap-2">
                            <span className="w-7 h-7 rounded-full bg-[#E15B39]/5 border border-[#E15B39]/15 flex items-center justify-center text-[#E15B39]">
            ...
            <div className="flex flex-col items-center justify-center p-12 text-gray-400 select-none min-h-[160px]">
              ...
              <p className="text-xs font-semibold">Personnel report content preview will compile here</p>
              <p className="text-[10px] text-gray-400 mt-1">
                Select a personnel member and optional date limits above then hit "Generate" to visualize request logs metrics.
              </p>
            </div>
          )}
        </div>
